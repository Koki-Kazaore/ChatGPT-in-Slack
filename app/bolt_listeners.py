...
def display_image_generation_result(
    ...
    text = ""
    try:
        prompt = extract_state_value(payload, "image_generation_prompt").get("value") or ""
        selected_option = extract_state_value(payload, "size").get("selected_option") or {}
        size = selected_option.get("value")
        quality = (
            extract_state_value(payload, "quality").get("selected_option").get("value")  # pytype: disable=attribute-error
            if extract_state_value(payload, "quality").get("selected_option")
            else None
        )
        style = (
            extract_state_value(payload, "style").get("selected_option").get("value")  # pytype: disable=attribute-error
            if extract_state_value(payload, "style").get("selected_option")
            else None
        )
        model = context.get(
...
def ack_chat_from_scratch_modal_submission(
    ...
    prompt = extract_state_value(payload, "prompt").get("value") or ""
    text = "\n".join(map(lambda s: f">{s}", prompt.split("\n")))
